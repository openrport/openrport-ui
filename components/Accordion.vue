...
<!--    </div>-->
  <div class="absolute inset-x-0 bottom-0 top-32">
    <div v-for="(item, index) in items" :key="index" class="w-full px-1">
      <div @click="toggle(index, item.id)"  :class="[
        ...
        <div class="info w-full pl-6">
          <div class="inline-flex w-max cursor-pointer min-w-full">
            <div class="text-sm font-normal text-current">{{ item.title }}</div>
          </div>
        </div>

        <div v-if="isActive(index)" class="relative flex w-6 items-center justify-center pt-0.5">
          <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" class="icon rotate-180 transform text-current transition-transform">
            <path fill="currentColor" d="M6.7 14.7q-.275-.275-.275-.7t.275-.7l4.6-4.6q.15-.15.325-.212T12 8.425q.2 0 .388.075t.312.2l4.6 4.6q.275.275.275.7t-.275.7q-.275.275-.7.275t-.7-.275L12 10.8l-3.9 3.9q-.275.275-.7.275t-.7-.275Z"></path>
          </svg>
        </div>
      </div>
      <div v-if="isActive(index)" class="relative z-10 w-full pb-2 pl-5 after:absolute after:-top-4 after:bottom-0 after:left-3 after:flex after:border-l-2 after:border-dashed after:border-primary after:content-['']">
        <div class="absolute inset-0 -top-4 z-30 flex items-center justify-center bg-white/50 backdrop-blur-sm" style="display: none;">
          <div class="flex w-full justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="32px" height="32px" viewBox="0 0 24 24" class="icon primary-text animate-[spin_0.6s_linear_infinite]">
        ...
        <RouterLink v-if="listStyle === 1" v-for="(device, i) in deviceList"  :to="`/dashboard/${device.id}`" class="mt-2 flex min-h-10 w-full max-w-full flex-100 cursor-pointer items-center overflow-hidden rounded border border-r-[3px] px-1 py-1 pr-2 bg-theme" :class="{ 'border-r-error' : device.disconnected_at !== null , 'border-r-success': device.disconnected_at === null}">
          <div class="client-icon ml-1 mr-2 flex w-10 items-center justify-center" style="opacity: 1; transform: scale(0.8);">
              <ClientIcon :os="device.os" :os_family="device.os_family" :os_full_name="device.os_full_name" :os_kernel="device.os_kernel"/>
          </div>
          <div class="flex flex-1 flex-col flex-wrap text-vtd-primary-500 dark:text-vtd-primary-400">
            <div class="truncate text-sm font-normal">{{ device.name }}</div><!----><!---->
          </div>
        </RouterLink>

        <RouterLink v-if="listStyle === 2 || listStyle === 3" v-for="(device, i) in deviceList"  :to="`/dashboard/${device.id}`" class="mt-2 flex min-h-10 w-full max-w-full flex-100 cursor-pointer items-center overflow-hidden rounded border border-r-[3px] px-1 py-1 pr-2 bg-theme" :class="{ 'border-r-error' : device.disconnected_at !== null , 'border-r-success': device.disconnected_at === null}">
          <div class="client-icon ml-1 mr-2 flex w-10 items-center justify-center" style="opacity: 1; transform: scale(1.2);">
            <ClientIcon :os="device.os" :os_family="device.os_family" :os_full_name="device.os_full_name" :os_kernel="device.os_kernel"/>
          </div>
          <div class="flex flex-1 flex-col flex-wrap text-vtd-primary-500 dark:text-vtd-primary-400">
            <div class="truncate text-sm font-normal">{{ device.name }}</div>
            <div class="truncate py-0.5 text-sm font-normal text-theme-inverted-lightinverted">{{ device?.ipv4 && device.ipv4?.length ? device.ipv4[0] : ''}}</div>
            <div v-if="listStyle === 3 && device?.tags?.length" class="relative flex h-5 w-full">
              <div class="absolute bottom-0 left-0 right-1 z-10 flex w-auto items-center gap-1 truncate">
                <span v-for="tag in device?.tags" class="whitespace-nowrap rounded bg-slate-200 px-3 py-0 text-sm text-theme-inverted">{{ tag }}</span>
              </div>
              <!---->
            </div>
          </div>
