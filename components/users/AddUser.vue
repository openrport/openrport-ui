<template>
    <dialog :is-active="is_active" :title="action === 'create' ? 'New User' : 'Edit User'" @close="closeModal"
        save-button-text="Save" @save="save">
        <div
            ...
            <div class="tw-relative">
                <div class="tw-flex tw-flex-col tw-mt-4">
                    <div class="tw-relative tw-mt-1 tw-flex tw-w-full tw-flex-col">
                        ...
                        <label
                            class="tw-order-1 tw-flex tw-pb-1 tw-pl-1 tw-text-theme-label peer-focus:tw-text-primary tw-text-sm tw-font-normal">Username</label>
                    </div>
                </div>
                <div class="tw-flex tw-flex-col tw-mt-4">
                    <div class="tw-relative tw-mt-1 tw-flex tw-w-full tw-flex-col">
                        <label
                            class="tw-flex tw-pb-1 tw-pl-1 tw-text-theme-label peer-focus:tw-text-primary tw-text-sm tw-font-normal">Groups</label>
                        <Multiselect mode="tags" v-model="user.groups" :close-on-select="false" :create-option="false"
                            :searchable="true" :options="groups?.map(item => item.name)" />

                    </div>
                </div>
                <div class="tw-mt-4 tw-pl-0.5 tw-text-sm tw-text-theme-inverted">Password</div>
                ...
                <div class="tw-flex tw-flex-col tw-mt-4">
                    <div class="tw-relative tw-mt-1 tw-flex tw-w-full tw-flex-col">
                        <input v-model="user.confirmPassword"
                            ...
                            type="text" autocomplete="off">
                        <label
                            class="tw-absolute tw-left-1.5 tw-top-1.5 tw-z-10 tw-origin-[0] -tw-trangray-y-4 tw-scale-75 tw-transform tw-px-1 tw-text-theme-label tw-duration-300 peer-placeholder-shown:tw-top-1/2 peer-placeholder-shown:-tw-trangray-y-1/2 peer-placeholder-shown:tw-scale-100 peer-focus:tw-top-1.5 peer-focus:-tw-trangray-y-4 peer-focus:tw-scale-75 peer-focus:tw-px-1 tw-text-theme-label peer-focus:tw-text-primary tw-bg-theme tw-text-sm tw-font-normal">Confirm
                            password</label>
                    </div>
                </div>
            </div>
