<template>
	<div class="hidden lg:flex lg:flex-shrink-0">
		<div :class="['relative z-40 flex flex-col transition-all', isNavBarOpen ? 'w-45': 'w-20']">
			<div class="flex min-h-0 flex-1 flex-col bg-primary-800">
				<div class="flex-1">
					<div class="flex items-center justify-center bg-primary-800 py-4">
						<div class="flex justify-center py-6 text-white">
							<span>
								<svg
									height="15px"
									version="1.1"
									...
									>
										...
										>
											...
											>
												<path
													id="Rectangle"
													d="M17.5217391,0 C19.4427296,0 21,1.52630199 21,3.40909091 L21,11.5909091 C21,13.473698 19.4427296,15 17.5217391,15 L8.47826087,15 C6.55727044,15 5,13.473698 5,11.5909091 L5,11 L8.478,11 L8.47826087,11.5909091 L17.5217391,11.5909091 L17.5217391,3.40909091 L8.47826087,3.40909091 L8.478,4 L5,4 L5,3.40909091 C5,1.52630199 6.55727044,0 8.47826087,0 L17.5217391,0 Z"
													fill-rule="nonzero"
												/>
												<polygon
													id="Path"
													fill-rule="nonzero"
													points="13 6 13 9 0 9 0 6"
												/>
												<rect
													id="Rectangle"
													height="3"
					...
					<nav class="flex flex-col items-center space-y-3 px-3 py-6">
						<nuxt-link
							to="/dashboard"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							<svg
								data-v-ca945699=""
								xmlns="http://www.w3.org/2000/svg"
							...
							<span v-if="isNavBarOpen">Clients</span>
						</nuxt-link>
						...
						>
							...
							<span v-if="isNavBarOpen">Tunnels</span>
							<!-- <span
						...
						>
							...
							<span v-if="isNavBarOpen">Inventory</span>
							<!-- <span
						...
						>
							...
							<span v-if="isNavBarOpen">Documentation</span>
						</nuxt-link>
						<nuxt-link
							to="/commands"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							>
								<g fill="currentColor">
									<path
										d="M2 6a3 3 0 0 1 3-3h14a3 3 0 0 1 3 3v12a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V6zm3-1a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1H5zm1.617 2.076a1 1 0 0 1 1.09.217l4 4a1 1 0 0 1 0 1.414l-4 4A1 1 0 0 1 6 16V8a1 1 0 0 1 .617-.924zM8 10.414v3.172L9.586 12L8 10.414zM12 16a1 1 0 0 1 1-1h4a1 1 0 1 1 0 2h-4a1 1 0 0 1-1-1z"
									/>
								</g>
							</svg>
							<span v-if="isNavBarOpen">Commands</span>
						</nuxt-link>
						<nuxt-link
							to="/scripts"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							<span v-if="isNavBarOpen">Scripts</span>
						</nuxt-link>
						<nuxt-link
							to="/schedules"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							<span v-if="isNavBarOpen">Schedules</span>
						</nuxt-link>
						<nuxt-link
							to="/profile"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							<span v-if="isNavBarOpen">Settings</span>
						</nuxt-link>
						<nuxt-link
							to="/library"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							<span v-if="isNavBarOpen">Library</span>
						</nuxt-link>
						<nuxt-link
							to="/audits"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
						>
							...
							<span v-if="isNavBarOpen">Audit Logs</span>
						</nuxt-link>
						<a
							href="#"
							:class="['items-start px-4 flex w-full rounded-lg py-2 text-primary-50 transition-all hover:bg-primary-700 transition-all', isNavBarOpen ? 'gap-x-4':'']"
				...
				<div class="flex flex-shrink-0 flex-col items-center gap-y-4 pb-5">
						...
						>
							<span class="flex items-center justify-center gap-x-2">
								<svg
									...
									height="20px"
									viewBox="0 0 24 24"
									class="icon rotate-90 transition-all"
								>
									<path
										fill="currentColor"
					...
					<a
						href="/profile"
						class="router-link-active router-link-exact-active w-full flex-shrink-0"
						aria-current="page"
					>
						<div class="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-primary-700">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								width="24px"
								height="24px"
								viewBox="0 0 256 256"
								class="icon mx-auto block h-6 w-6"
							>
								<path
									fill="currentColor"
